...
if (isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == 'supprimer') {
    $id = (int)$_GET['id'];
    $delete = $conn->prepare("DELETE FROM appliances WHERE id = ?");
    $delete->execute([$id]);
    $succesMsg = "Appliance supprimée avec succès";
}


$res = $conn->prepare("SELECT * FROM appliances ORDER BY id DESC");
$res->execute();
?>
...
<head>
    ...
    <title>Liste des Appliances - ONEE BO</title>
        ...
        <div class="tab"> <table>
            <thead>
                <tr>
                    ...
                    <th scope="col">Capacité disque brut (Go)</th>
                    <th scope="col">Capacité disque utilisé (Go)</th>
            ...
            <tbody>
                <?php while ($row = $res->fetch()) { ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['Capacite_Disque_Brut']) ?></td>
                        <td><?= htmlspecialchars($row['Capacite_Disque_Utilise']) ?></td>
                        ...
                        <td>
                            <a href="modifierAppliance.php?id=<?= $row['id'] ?>">Modifier</a>
                            <a href="listeAppliance.php?id=<?= $row['id'] ?>&action=supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette appliance?');">Supprimer</a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>
