...
if (isset($_GET['id']) && isset($_GET['action']) && $_GET['action'] == 'supprimer') {
    $id = (int)$_GET['id'];
    $delete = $conn->prepare("DELETE FROM réseau WHERE id = ?");
    $delete->execute([$id]);
    $succesMsg = "Réseau supprimé avec succès.";
}


$res = $conn->prepare("SELECT * FROM réseau ORDER BY id DESC");
$res->execute();
?>
        ...
        <div class="tab">        <table>
            <thead>
                <tr>
                    ...
                    <th scope="col">Nombre de Ports Totaux</th>
                    <th scope="col">Nombre de Ports Utilisés</th>
            ...
            <tbody>
                <?php while ($row = $res->fetch()) { ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['Nombre_Ports_Totaux']) ?></td>
                        <td><?= htmlspecialchars($row['Nombre_Ports_Utilises']) ?></td>
                        ...
                        <td>
                            <a href="modifierRéseau.php?id=<?= $row['id'] ?>">Modifier</a>
                            <a href="listeRéseau.php?id=<?= $row['id'] ?>&action=supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette appliance?');">Supprimer</a>
